...
def load_data_2017_HH(inputPath,channelInTree,variables,criteria,bdtType) :
    ...
    for folderName in keys :
        ...
        if 'signal_ggf' in folderName or 'signal_vbf' in folderName:
            ...
            procP1=[w for w in glob.glob(inputPath+"/"+folderName+"*/"+"central/"+"*.root") if not os.path.basename(w).startswith('WZ') and w.find('part') ==-1]
            list=procP1
        ...
        for ii in range(0, len(list)) :
            ...
            if tree is not None :
                ...
                if "evtLevelSUM_HH_bb2l_res" in bdtType or "evtLevelSUM_HH_bb1l_res" in bdtType :
                        foundMass = False
                        for mass in masses :
